<template>
  <el-container style="overflow: hidden">
    <el-main class="main">
      <p style="line-height: 20px;justify-self: center; font-weight: bolder" class="glass-text">散点图</p>
      <div ref="point_chart" style="width: 800px; height: 600px;justify-self: center" class="glass-container"></div>
    </el-main>
    <el-aside width="400px" class="aside">
      ...
      <el-upload
          class="upload-demo"
          ref="upload"
          action="http://localhost:8000/point_chart"
          :auto-upload="false"
...
export default {
  ...
  mounted() {
    this.chartInstance = echarts.init(this.$refs.point_chart);
    // 使用刚指定的配置项和数据显示图表。
  ...
  methods: {
    handleUploadSuccess(response) {
      ...
      this.option.tooltip.formatter = (params) => {
        return `${this.headers[0]}: ${params.value[0]}<br>${this.headers[1]}: ${params.value[1]}`;
      };
      this.chartInstance.setOption(this.option);
    },
    submitUpload() {
    ...
    async downloadFile() {
      const filename = '6.xlsx';  // 需要下载的 Excel 文件名
      try {
