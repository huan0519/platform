<template>
  <el-container style="overflow: hidden">
    <el-main class="main">
      <p style="line-height: 20px;justify-self: center;font-weight: bolder" class="glass-text">3DChart</p>
      <div ref="_3DChart" style="width: 800px; height: 600px;justify-self: center" class="glass-container"></div>
    </el-main>
    <el-aside width="400px" class="aside">
      ...
      <el-upload
          class="upload-demo"
          ref="upload"
          action="http://localhost:8000/3DChart"
          :auto-upload="false"
      ...
      <div class="input-container">
        ...
        <div class="input-row">
          <label for="z_name">z轴名称:</label>
          <el-input id="z_name" style="width: 350px" type="text" v-model="z_name"></el-input>
        </div>
        <div class="input-row">
...
export default {
    ...
    return {
      ...
      option: {
        ...
        xAxis3D: {
          name:'',
          type: 'category',
          data: [] // Placeholder for hours
        },
        yAxis3D: {
          name:'',
          type: 'category',
          data: [] // Placeholder for days
        },
        zAxis3D: {
          name:'',
          type: 'value'
        ...
        series: [
          {
            type: 'bar3D',
            data: [], // Placeholder for 3D data points
            shading: 'lambert',
            label: {
              fontSize: 16,
              borderWidth: 1
  ...
  mounted() {
    this.chartInstance = echarts.init(this.$refs._3DChart);
    // 使用刚指定的配置项和数据显示图表。
    this.chartInstance.setOption(this.option);
  },
  methods: {
    handleUploadSuccess(response) {
      const { days, hours, data } = response;
      this.option.yAxis3D.data = days;
      this.option.xAxis3D.data = hours;
      if (data && Array.isArray(data)){
        this.option.series[0].data = data.map(item => ({
          value: [item[1], item[0], item[2]] // Converting [y_index, x_index, value] to [hour, day, value]
        }));
        // // 计算 data 的平均值并取整
        // // 计算整数的和，然后求平均值
        // const totalValue = data.reduce((sum, item) => sum + Math.round(item[2]), 0);
        // const averageValue = totalValue / data.length; // 求平均值
        //
        // console.log(averageValue);
        // this.option.visualMap.max=averageValue;
        // 计算整数的中位数
        const roundedData = data.map(item => Math.round(item[2])); // 先将值取整
        roundedData.sort((a, b) => a - b); // 排序

        let medianValue;
        const middle = Math.floor(roundedData.length / 2);

        if (roundedData.length % 2 === 0) {
          // 偶数个数时，中位数是两个中间数的平均值
          medianValue = (roundedData[middle - 1] + roundedData[middle]) / 2;
        } else {
          // 奇数个数时，取中间的数
          medianValue = roundedData[middle];
        }
        console.log(medianValue);
        this.option.visualMap.max=medianValue;
        // 使用新的配置项和数据更新图表
        this.chartInstance.setOption(this.option);
      }else {
        console.error('chartData is not an array or is undefined');
      }
    },
    submitUpload() {
      this.option.xAxis3D.name=this.x_name;
      this.option.yAxis3D.name=this.y_name;
      this.option.zAxis3D.name=this.z_name;
