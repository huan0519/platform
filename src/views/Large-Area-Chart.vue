<template>
  <el-container>
    <el-main class="main">
      <p style="line-height: 20px;margin-left: 20px; font-weight: bolder">Large Area Chart</p>
      <div ref="Large_Area_Chart" style="width: 850px; height: 500px"></div>
    </el-main>
    <el-aside width="400px" class="aside">
      ...
      <el-upload
          class="upload-demo"
          ref="upload"
          action="http://localhost:8000/Large_Area_Chart"
          :auto-upload="false"
...
export default {
  ...
  data() {
    return {
      ...
      option: {
        ...
        dataZoom: [
          {
            type: 'inside', // 可以在图表内部进行缩放
            start: 0,
            end: 10
          },
          {
            type: 'slider', // 提供滑动条外部缩放
            start: 0,
            end: 10
          }
        ],
        series: [
          {
            ...
            // 启用渐进式渲染
            progressive: true,
            progressiveThreshold: 200, // 当数据点数超过 5000 时，启用渐进式渲染
            data: [] // 使用已经优化的数据
          }
        ]
      },
  ...
  mounted() {
    this.chartInstance = echarts.init(this.$refs.Large_Area_Chart);
    // 使用刚指定的配置项和数据显示图表。
    this.chartInstance.setOption(this.option);
  },
  methods: {
    handleUploadSuccess(response) {
      const { date, data, data_title } = response;
      this.option.xAxis.data = date;  // 设置横轴数据
      this.option.series[0].data = data;  // 设置纵轴数据
      this.option.series[0].name = data_title;
      // 使用新的配置项和数据更新图表
      this.chartInstance.setOption(this.option);
    },
    submitUpload() {
    ...
    async downloadFile() {
      const filename = '4.xlsx';  // 需要下载的 Excel 文件名
      try {
