<template>
  <el-container>
    <el-main class="main">
      <p style="line-height: 20px;margin-left: 20px; font-weight: bolder">玫瑰图</p>
      <div ref="rose_chart" style="width: 850px; height: 600px"></div>
    </el-main>
    <el-aside width="400px" class="aside">
      ...
      <el-upload
          class="upload-demo"
          ref="upload"
          action="http://localhost:8000/rose_chart"
          :auto-upload="false"
...
export default {
  ...
  mounted() {
    this.chartInstance = echarts.init(this.$refs.rose_chart);
    // 使用刚指定的配置项和数据显示图表。
  ...
  methods: {
    handleUploadSuccess(response) {
      // 更新数据到图表
      this.option.series[0].data = response; // 设置箱线图数据
      // 使用新的配置项和数据更新图表
      this.chartInstance.setOption(this.option);
    },
    async submitUpload() {
    ...
    async downloadFile() {
      const filename = '7.xlsx';  // 需要下载的 Excel 文件名
      try {
